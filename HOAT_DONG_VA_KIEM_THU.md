...
Luồng "phân tích 1 lần": bấm ⊹ → app hiện pill **"Giữ nguyên máy — đang tìm
điểm chụp đẹp…"** → khi chủ thể ổn định 3 frame liên tiếp, AI **chốt một lần
duy nhất** chủ thể (khoá tracking ID) và điểm bố cục đẹp → hiện **nốt tròn**
đánh dấu điểm ngắm + thông báo "Đã tìm thấy điểm chụp đẹp". Từ đó đích không
đổi nữa — người dùng chỉ di máy cho dấu + trùng nốt tròn. Mất dấu chủ thể →
báo "bấm ⊹ để phân tích lại". Chi tiết pipeline:
...
5. **Tính lời khuyên** (`adviseComposition`): điểm đích = giao điểm
   ...
   thể cách đích ≤5% → `isAligned`. Từ đó tính **điểm ngắm** (`aimPoint`):
   vị trí sao cho khi nó về giữa màn hình thì chủ thể rơi đúng điểm đích.
6. **Overlay kiểu ngắm:** một **dấu + cố định giữa màn hình** và một
   **nốt tròn** tại điểm ngắm (AI đã phân tích là điểm chụp đẹp nhất).
   Người dùng chỉ cần di máy cho dấu + trùng nốt tròn. Nốt ra ngoài khung
   thì ghim vào mép để vẫn thấy hướng di. Trắng = bình thường, **vàng** =
   đã khoá chủ thể, **xanh + rung nhẹ + vòng xác nhận quanh dấu +** = trùng.
...
- [ ] Bật ⊹, chĩa vào 1 vật thể rõ (cốc nước, người) đặt lệch góc: di máy
      cho **dấu + trùng nốt tròn** → lúc trùng, vật thể phải nằm **đúng giao
      điểm 1/3 khung** (bật grid lên đối chiếu). Nếu lệch cố định → giả định
      center-crop sai; lệch kiểu hoán đổi trục → hệ toạ độ xoay sai.
      Sửa tại `mapImageRectToView` / `subject_detector.dart`.
- [ ] Thử cả 3 tỉ lệ khung 3:4 / 1:1 / 9:16 — kết quả trùng vẫn đúng.
- [ ] **Camera trước**: di mặt sang trái → nốt tròn di chuyển đúng chiều
      trực giác trên màn hình (kiểm tra giả định mirror). Ngược → đổi `mirrorX`.

**Hành vi dẫn hướng (luồng phân tích 1 lần):**
- [ ] Bấm ⊹ → pill "Giữ nguyên máy — đang tìm điểm chụp đẹp…" hiện ngay;
      giữ máy ~1s vào cảnh có chủ thể → pill biến mất, có rung nhẹ +
      thông báo "Đã tìm thấy điểm chụp đẹp", nốt tròn xuất hiện.
- [ ] Sau khi chốt, **nốt tròn không nhảy sang điểm đích khác** khi di máy
      (đích cố định) — chỉ trôi mượt theo cảnh.
- [ ] Di máy cho dấu + trùng nốt → **chuyển xanh + rung + vòng xác nhận**.
- [ ] Nốt nằm ngoài khung nhìn → ghim ở mép đúng phía cần di máy.
- [ ] Che chủ thể / lia máy đi chỗ khác → nốt ẩn + thông báo "Mất dấu chủ
      thể — bấm ⊹ để phân tích lại" (hiện đúng 1 lần, không spam).
- [ ] Chĩa vào tường trắng rồi bấm ⊹ → pill hiện mãi (không chốt bừa);
      lia sang cảnh có chủ thể → chốt được.
- [ ] **Long-press** lên vật khác trong lúc dẫn hướng → nốt chuyển **vàng**,
      đích chốt lại cho vật đó; long-press vùng trống → phân tích lại từ đầu.
- [ ] Flip camera / ra vào app khi đang bật ⊹ → tự phân tích lại từ đầu.
